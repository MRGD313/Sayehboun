...
from sayehboun_tuner.prompt_versions import normalize_version, versions_dir


def manifest_path(tuner_root: Path) -> Path:
    return versions_dir(tuner_root) / "judging_manifest.json"
...
def prompt_file_for_version(tuner_root: Path, version: str) -> Path:
    ...
    return versions_dir(tuner_root) / f"judging_{version}.txt"
...
def register_version(
    ...
    *,
    ...
    version = normalize_version(version)
    dest = versions_dir(tuner_root) / f"judging_{version}.txt"
...
def set_current_version(tuner_root: Path, version: str) -> None:
    ...
    if version not in manifest.get("versions", {}):
        raise ValueError(f"Version {version} is not registered in judging_manifest.json")
    manifest["current"] = version
    save_manifest(tuner_root, manifest)


def update_env_judging_prompt_version(sayehboun_root: Path, version: str) -> None:
    version = normalize_version(version)
    ...
    for idx, line in enumerate(lines):
        if line.startswith("JUDGING_PROMPT_VERSION="):
            lines[idx] = f"JUDGING_PROMPT_VERSION={version}"
            updated = True
            break
    if not updated:
        lines.append(f"JUDGING_PROMPT_VERSION={version}")
    env_path.write_text("\n".join(lines) + "\n", encoding="utf-8")


def write_current_marker(tuner_root: Path, version: str) -> None:
    path = versions_dir(tuner_root) / "JUDGING_CURRENT"
